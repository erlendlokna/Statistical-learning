...
author: "Thomas Rødland, Erlend Lokna"
...
plot(summary(regfit.fwd)$adjr2, xlab='No. of Variables',ylab='R^2',
     main="forward",type='b', xlim=c(1,4), ylim=c(0.54, 0.71))

```

```{r}
plot(summary(regfit.bcwd)$adjr2, xlab='No. of Variables',ylab='R^2', ylim=c(0.54, 0.71),
...
We can clearly see that the PSLR method shrinks the MSEP quicker for fewer components when comparing to the PCR method. PCR is a unsupervised method while PLSR is supervised. Meaning that for a data set with correlated values, the PLSR will perform better. 
...
We see that there is a correlation between X2 and X3, as shown by the blue squares in the figure above. 
...
using the gam-function to create an adaptive model:
```{r, eval=TRUE}
adpt_mod <- gam(medv ~rm  + s(ptratio, df=3) + lo(lstat, degree=2, span=1), data=boston.train)
```
...
## a)
i)
First of all we observe that the "logged GDP", "social support", "healthy life expectancy" and "ladder score" practically have the same direction in the plot above. This implies a correlation, and we can assume that each one effects the other. We also observe that "freedom to make life choices" and "perception of corruption" is close to opposite in direction. This may be interpreted as countries having more freedom experience less corruption. Also there is no relation between corruption and generosity as the angle between the two directions are sufficient. The same can be said about the generosity and "logged GDP", "social support", "healthy life expectancy" and "ladder score".
